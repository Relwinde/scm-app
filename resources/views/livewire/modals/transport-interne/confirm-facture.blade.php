<div>
    <div class="card form-input-elements">
        <div class="card-header justify-content-between">
            <h3 class="mb-0 card-title"><b>Confirmation de la fature du dossier {{ $dossier->numero }}</b></h3>
        </div>
        <form wire:submit.prevent="save">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6 ">
                        <div class="mb-3">
                            <label for="formFile" class="form-label">Entrez le numéro de facture SCM</label>
                            <input class="form-control" type="text" wire:model="numero_facture" required>
                            @error('numero_facture')<div class="error-message"> {{ $message }} </div>@enderror
                        </div>
                    </div>
                    <div class="col-md-6 ">
                        <div class="mb-3">
                            <label for="formFile" class="form-label">Selectionnez la facture SCM (format PDF)</label>
...
@script
    ...
    <script>
        $wire.on('facture-confirmed', () => {
            (function () {
                $(function () {
                    return $.growl({
                        title: "Succès :",
                        message: "La Facture SCM a été importé avec succès."
                    });
                });
            }).call(this);
        });
